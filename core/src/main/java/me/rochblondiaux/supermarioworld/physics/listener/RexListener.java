...
public class RexListener  implements ContactListener {
    ...
    public void beginContact(Contact contact) {
        ...
        if (entityA == null
            ...
            || (!(entityA instanceof Rex) && !(entityB instanceof Rex)))
            return;

        Player player = entityA instanceof Player ? (Player) entityA : (Player) entityB;
        Rex collectable = entityA instanceof Rex ? (Rex) entityA : (Rex) entityB;
        ...
        // Interact
        collectable.onCollision(player, direction);
